...
> 状态：项目中使用
>
> 验证状态：待验证。用户课程记录描述已实现同名同级合并、物品构筑标签、方向受限邻接扫描和接口点 UI；本环境完成静态审阅与文档验证，未重新运行 Unity。
>
> 日期：2026-07-28
>
...
第 14 课阶段 `GetWorldConnectableSides()` 直接返回本地接口，说明当时接口方向暂未随物品旋转变化。第 28 课已经兑现这个命名：`Item` 通过四状态 `RotationState` 将 `LocalConnectableSides` 换算为当前世界方向，扫描器继续只问“世界方向上有没有接口”。
...
> 后续演进：第 15 课已将背包内武器物品映射到场景自动武器实体，并用实例级激活角标显示哪一件物品正在驱动战斗，详见[背包武器激活](backpack-weapon-activation.md)。第 21 课已把 DualWield 从候选邻接推进到真实战斗收益，详见[构筑最小兑现](build-payoff-dual-wield.md)。第 22 课继续把物品标签、方向、价值和效果强度上移到 `LootEntry`，并接入可堆叠 `FireRateBoost`，详见[内容面铺开](content-expansion-fire-rate-boost.md)。第 26 课进一步让合并等级兑现为当前价值和效果收益，详见[合并升级收益兑现](merge-upgrade-reward-payoff.md)。第 28 课修正旋转后的本地 / 世界接口方向和正反向匹配，详见[旋转邻接方向修正](rotation-adjacency-direction-fix.md)。第 29 课继续让合并等级影响武器实例倍率，并补上合并后 `OnChanged` 语义刷新，详见[武器稀有度与等级差异](weapon-rarity-and-level-scaling.md)。
...
| Local / World 命名不兑现 | `GetWorldConnectableSides()` 暂未处理旋转 | 第 28 课已用 `RotationState` 和 `GetWorldSides()` 统一转换 |
