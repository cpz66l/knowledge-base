...
> 状态：V0.2 背包交互补丁已记录，准备进入合并升级与邻接联动
>
> 首次记录：2026-07-20
>
> Unity 版本：Unity6.3
...
- 第 14 课推进合并升级和第一条邻接联动。
- 为 `TargetRegistry` 增加场景/Play Mode 清理、按阵营计数和失效目标处理。
- 补做低帧率、多 Collider、命中缓冲区满载、刷怪点合法性、跨池归还、批量拾取、拖拽中断、交互拾取失败、宝箱生成和丢弃回捡测试。
- 使用 Profiler 验证预热量、扩容次数、索敌、物理查询、UI 重绘、经验球吸附、宝箱生成、散落协程和 GC Alloc。
