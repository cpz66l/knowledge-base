...
原始记录描述的运行结果是：敌人会在视野内追击玩家，进入攻击范围后周期性造成近战伤害；敌人生命值归零后执行死亡流程；中立危险区可以同时作用于玩家和敌人。此次 Inbox 只包含课程笔记，没有完整脚本、场景、Prefab 或运行截图，因此这些结果属于用户的项目实践记录，不是本知识库环境重新运行得到的结果。

## 最小状态分支

当前行为可以明确拆成三个状态，而不是“两态”：</n+
```text
距离 > viewRange
  └─ 待机
...
对于“创建一次、死亡后销毁”的敌人，这条路径能够表达订阅和清理。第 3 课改成停用并准备复用后，这组生命周期就不再完整，详见[目标注册表、自动武器与投射物](target-registry-and-auto-weapon.md)。
...
当死亡从 `Destroy` 改成 `SetActive(false)` 后，`Start` 不会在再次激活时重跑。如果死亡时已经执行 `health.OnDeath -= Die`，下一次启用后不会重新订阅；同时生命值和 `IsDead` 也需要重置。第 3 课当前只完成了“停用”这一步，还没有形成可复用敌人的完整对象池闭环。
...
本次只完成了文档静态复核，没有可运行的完整 Unity 脚本与场景，因此不能声明这些边界已经通过。
