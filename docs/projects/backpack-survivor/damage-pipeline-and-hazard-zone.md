...
| `HazardZone` | 缓存 Trigger 内玩家阵营目标，并按 tick 施加环境伤害 |
...
生命值组件不直接引用血条、飘字、掉落或任务系统。当前阶段只验证了发布者一侧；还没有实际订阅者，因此“订阅与取消订阅生命周期已经正确”仍待下一课验证。
...
目前只有一种危险区，没有足够案例证明多个区域共享稳定行为。先通过字段配置阵营、伤害和间隔；等出现两到三个真实变体后，再根据重复代码提炼基类或策略，避免为了预测未来而过早抽象。
...
- 接入敌人追击和近战攻击，让第二种伤害来源复用 `IDamageable`。
- 首次真实订阅 `OnDeath`，验证死亡动画、掉落和事件解绑。
- 增加 `CurrentHp` / `MaxHp` 只读接口，为 UI 订阅提供状态。
- 根据多 Collider 和低帧率测试结果修正 `HazardZone`。
- 记录 Unity 版本、场景结构、Collider 布局和测试证据。
