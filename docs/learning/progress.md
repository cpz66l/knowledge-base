...
> 本周记录入口：[2026-W34](weekly/2026-W34.md)
...
|---|---|---|---|---|
| Unity / C# 项目能力 | 项目中使用 | Backpack Survivor V0.3.6、项目页、检查清单、正式包用户验收记录、2026-W33 第 3 批面试复盘、第 5 批 V0.3 项目复盘和 2026-W34 第 1 批项目复盘 | 当前最强项，能支撑实习投递的项目主线；项目表达已从 V0.2 演示包推进到升级候选池、邻接架构、背包构筑收益、内容池、音频反馈和设置菜单 | 整理 3 分钟项目讲稿、6 个技术亮点、5 个 Bug 复盘和 1 条性能判断链路；W34 优先把 V0.3 架构、音频和设置系统讲成面试故事 |
| C# 工程能力 | 学习中 / 项目中使用 | `interface`、事件、纯 C# 背包、对象池、asmdef、UGUI 投影、Build 记录 | 有真实项目证据，但系统化 GC、异步、日志、异常策略仍待补 | 用项目复盘补“为什么这样设计”和“如何验证” |
| 算法 / LeetCode | 学习中 | 当前记录 33 道题，其中 5 道 C# / C++ 双语对照；2026-W34 新增 LC 102、LC 108 两道 C++ 二叉树练习 | W34 已开始保持刷题，但目前新增题仍集中在二叉树；仍需要稳定日刷、错题复盘和更广题型覆盖 | 后续刷题优先用 C++，每题保留复杂度、错点和提交证据；W34 剩余题目优先补栈 / 堆 / 二分 / 回溯入口 |
| C++ | 学习中 | C++ 基础语法 / STL / OOP / 内存笔记、部分 LeetCode；新增指针/引用、类与对象、栈 / 堆、继承 / 多态、虚析构、浅拷贝风险、`enum class`、结构化绑定、`map`、智能指针概念和九道 C++ 练习 | 能起步刷题，已接触 `vector`、`string`、`unordered_map`、`map`、`stack`、`queue`、指针、引用、`nullptr`、初始化列表、`new/delete`、继承、虚函数和智能指针概念，但生命周期、浅拷贝、RAII 和编译运行验证仍是短板 | 用 LeetCode 和小程序补基础语法、常用 STL、指针 / 引用边界、对象生命周期、虚析构和智能指针；下一步必须补可运行输出 |
| 计算机网络 | 学习中 / 待实验 | 简历写“了解 TCP/UDP”；2026-W33 新增网络基础、网络层 / IP / 路由、TCP/UDP 和应用层协议笔记 | 已开始学习网络分层、交换方式、性能指标、五层模型、数据报服务、IPv4/CIDR、ARP/NAT/IPv6、RIP/OSPF/BGP、TCP/UDP、DNS/FTP/HTTP；但缺抓包、通信实验、子网计算和高频题复盘 | 补 DNS 查询、HTTP 抓包、TCP/UDP 高频问答、Socket、状态同步 / 帧同步，并至少画一张 URL 访问流程图 |
...
| 性能优化 | 学习中 / 项目中使用 | 对象池、NonAlloc、sqrMagnitude、Profiler 快扫、Build 颜色修复、`PickUpMagnet` 背包被动重复汇总挂账 | 有项目经验和取舍记录，但缺系统化前后数据；已能记录“暂不优化”的触发条件 | 准备一段面试表达：Profiler 如何判断“不重构”；后续用 V0.3 掉落量级验证 `PickUpMagnet` 是否需要共享缓存 |
| 面试表达 | 准备中 | 简历、项目复盘、路线总纲、[Backpack Survivor 面试复盘第 01 阶段](../reviews/2026/backpack-survivor-interview-stage-01.md) | 项目素材足，已开始把背包分层、拾取链路和 BUG-001 压缩成可讲版本；仍需继续口述训练 | 先做 1 分钟、3 分钟、10 分钟三版项目表达；下一轮从对象池和 `TargetRegistry` 开始模拟追问 |
...
- 知识库已记录 33 道 LeetCode 题，其中 5 道形成 C# / C++ 双语对照；2026-W34 新增 LC 102、LC 108 两道 C++ 二叉树练习题。
- 已有题型覆盖哈希、双指针、滑动窗口、子串、数组、矩阵和链表的部分内容。

仍待补强：

- 每日稳定刷题节奏。
- 图、回溯、二分、栈、堆、贪心、动态规划；二叉树已继续补层序和构造类题型，但 W34 后续要离开二叉树舒适区。
- 错题复盘和复杂度口头表达。
...
- 已整理运算符、控制流、函数、结构体、`enum class`、结构化绑定、`vector`、`string`、`map`、指针、引用、`nullptr`、`const` 指针组合、类与对象、初始化列表、`this`、继承、多态、虚析构、抽象类、栈 / 堆、`new/delete`、浅拷贝风险和智能指针概念的入门笔记。
...
- 已新增 V0.3 项目复盘证据：升级候选池、邻接效果架构、背包被动 / 邻接构筑收益分层、内容池价值平衡、基础音频系统、设置菜单和 `PickUpMagnet` 性能挂账。
...
根据当前基线，W34 不急着开新项目，重点是把 W33 的“已阅读 / 已整理”推进到“可运行 / 可口述 / 可验证”：

1. 算法恢复：每天 1 题，优先用 C++ 留下题解、复杂度、错点和提交证据。
2. C++ 基础：用链表 / 二叉树题继续补指针、引用、递归边界、`vector`、`string`、`unordered_map` 和 `stack`，同时写最小多态 / 虚析构小程序。
3. 项目表达：C# 主要通过 Backpack Survivor 工程训练，整理 1 分钟和 3 分钟讲稿；下一轮从 V0.3 升级候选池、邻接 Collector 和背包被动收益开始模拟追问。
4. 计网 / OS：每天小步学习高频面试内容；计网下一步先补 HTTP/HTTPS、DNS 查询与 TCP/UDP 高频问答，OS 先补进程 / 线程。
5. Shader：只做 1 个最小可观察效果的准备或实验，证据不足前不升级简历表述。
6. 进度系统：用第一篇周学习记录验证“计划 -> 执行 -> 证据 -> 复盘 -> 更新进度”的闭环。

具体计划见：[2026-W34 周学习记录](weekly/2026-W34.md)。
