...
|---|---|---|---|---|
| Unity / C# 项目能力 | 项目中使用 | Backpack Survivor V0.2、项目页、检查清单、正式包用户验收记录、2026-W33 第 3 批面试复盘 | 当前最强项，能支撑实习投递的项目主线；已开始把背包链路转成面试表达 | 整理 3 分钟项目讲稿、6 个技术亮点、5 个 Bug 复盘和 1 条性能判断链路；下一轮优先练对象池 |
| C# 工程能力 | 学习中 / 项目中使用 | `interface`、事件、纯 C# 背包、对象池、asmdef、UGUI 投影、Build 记录 | 有真实项目证据，但系统化 GC、异步、日志、异常策略仍待补 | 用项目复盘补“为什么这样设计”和“如何验证” |
| 算法 / LeetCode | 学习中 | 当前记录 30 道题，其中 5 道 C# / C++ 双语对照；2026-W33 新增 LC 138、LC 148、LC 94、LC 104、LC 226、LC 101 六道 C++ 练习 | 具备基础，已开始恢复 C++ 刷题节奏，但还没形成稳定日刷和错题体系 | 后续刷题优先用 C++，每题保留复杂度、错点和提交证据 |
| C++ | 学习中 | C++ 基础语法 / STL / OOP / 内存笔记、部分 LeetCode；新增指针/引用、类与对象、栈 / 堆和 LC 138、LC 148、LC 94、LC 104、LC 226、LC 101 C++ 练习 | 能起步刷题，已接触 `vector`、`string`、`unordered_map`、`stack`、`queue`、指针、引用、`nullptr`、初始化列表和 `new/delete`，但生命周期、悬空指针、递归边界和 RAII 仍是短板 | 用 LeetCode 和小程序补基础语法、常用 STL、指针 / 引用边界和对象生命周期 |
| 计算机网络 | 学习中 / 待实验 | 简历写“了解 TCP/UDP”；2026-W33 新增网络基础、网络层 / IP / 路由和 TCP/UDP 对比笔记 | 已开始学习网络分层、交换方式、性能指标、五层模型、数据报服务、IPv4/CIDR、ARP/NAT/IPv6、RIP/OSPF/BGP 和 TCP/UDP；但缺抓包、通信实验、子网计算和高频题复盘 | 补 TCP/UDP、HTTP/HTTPS、Socket、状态同步 / 帧同步高频题，并至少画一张封装流程图、子网计算记录或同步模型图 |
...
- 知识库已记录 30 道 LeetCode 题，其中 5 道形成 C# / C++ 双语对照；2026-W33 新增 LC 138、LC 148、LC 94、LC 104、LC 226、LC 101 六道 C++ 练习题。
...
- 错题复盘和复杂度口头表达。
- C++ 手写熟练度，尤其是随机指针、链表拆分、归并、二叉树镜像递归和空指针边界。
...
- 已整理运算符、控制流、函数、结构体、`vector`、`string`、指针、引用、`nullptr`、`const` 指针组合、类与对象、初始化列表、`this`、栈 / 堆和 `new/delete` 的入门笔记。

仍待补强：

- 指针、引用、对象生命周期、构造 / 析构的编译运行验证。
...
- 继续训练对象池、`TargetRegistry`、`AdjacencyRuleBook / Resolver`、`WaveDirector` 和 Profiler 快扫追问。
...
3. 项目表达：C# 主要通过 Backpack Survivor 工程训练，整理 1 分钟和 3 分钟讲稿；下一轮从对象池开始模拟追问。
4. 计网 / OS：每天小步学习高频面试内容；计网下一步先补 TCP/UDP 与 HTTP/HTTPS，OS 先补进程 / 线程。
