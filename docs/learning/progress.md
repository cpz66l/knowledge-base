...
| 算法 / LeetCode | 学习中 | 当前记录 24 道题，其中 5 道 C# / C++ 双语对照 | 具备基础，但还没形成稳定日刷和错题体系 | 从数组、哈希、双指针、滑动窗口、链表恢复每日 1 题 |
| C++ | 学习中 | C++ 路线记录为部分基础语法、部分 LeetCode | 能起步刷题，但指针、引用、生命周期、STL 和 RAII 仍是短板 | 用 C# 对比法补基础语法和常用 STL |
| 计算机网络 | 待系统学习 / 简历层了解 | 简历写“了解 TCP/UDP”；知识库网络专题仍主要是规划入口 | 不能写成掌握，只能作为待补面试基础 | 先学 TCP/UDP、HTTP、连接与可靠传输高频题 |
...
- 知识库已记录 24 道 LeetCode 题，其中 5 道形成 C# / C++ 双语对照。
...
1. 算法恢复：每天 1 题，留下题解和复杂度。
2. C++ 基础：用 C# 对比法补引用、指针、`vector`、`string`、`unordered_map`。
3. 项目表达：把 Backpack Survivor 整理成 1 分钟和 3 分钟讲稿。
4. 进度系统：用第一篇周学习记录验证“计划 -> 执行 -> 证据 -> 复盘 -> 更新进度”的闭环。
