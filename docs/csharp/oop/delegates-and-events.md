...
第 3 课把死亡改成 `SetActive(false)` 后验证了一个更重要的边界：如果只在 `Start` 订阅、在 `Die` 退订，那么对象再次启用时 `Start` 不会重跑，死亡处理不会恢复。第 5 课已将死亡订阅迁移到 `OnEnable` / `OnDisable`，并在池化取出时重置 `Health`，让订阅生命周期与激活期匹配。第 7 课进一步把掉落接到 `Die()`，并明确掉落只属于死亡事件，不属于 `OnReturnPool()`；否则一次死亡可能触发两次掉落。第 8 课让 `DropItem` 通过静态事件 `OnCollected` 广播拾取结果，临时 `PickupLogger` 只负责订阅；第 10 课正式由 `InventorySystem` 订阅这个事件，把掉落物自动放入背包。第 11 课继续把事件用于交互提示：`InteractDetector.OnTargetChanged` 只在最近目标变化时广播，`InteractPromptUI` 订阅后显隐提示，避免每帧轮询 UI。第 12 课让 `ChestSpawner` 订阅 `EnemyAI.OnEnemyDied`，把击杀事件转成宝箱生成机会；第 13 课新增 `OnInteractFailed`，让交互失败只广播反馈，不清除当前目标。第 9 课的 `InventoryGrid.OnChanged` 则把事件推进到纯 C# 数据层，第 10 课 UI 订阅后重绘投影，而不是每帧轮询或保存第二份玩法状态；第 15 课继续让 `BackpackWeaponActivator` 监听同一个 `OnChanged`，在背包变化后刷新场景武器实体；第 21 课又让同一变化入口驱动 `AdjacencyEffectResolver`，使 UI 和战斗都消费真实有效效果。第 16 课让 `GameSession` 同时订阅玩家死亡、经验拾取和暂停输入，再向 HUD 广播时间、经验和状态快照。第 17 课进一步让 `GameSession` 在升级时发布 `OnLevelUpChoiceRequested`，`LevelUpChoiceView` 订阅后刷新三选一按钮并把玩家选择回传。第 18 课则由 `WaveDirector` 发布 `OnWaveStageChanged`，`RunHudView` 订阅后只显示波次名和颜色。第 19 课继续让 `DamageFlashView`、`DamageNumberSpawner` 和 `PlayerHitFeedbackView` 消费 `Health.OnDamaged`，把同一个伤害事实投影成闪色、伤害数字、音效和震屏。第 20 课让 `GameSession.OnRunEnded` 发布 `RunResult`，`ResultView` 订阅后显示结算；同时 `GameSession` 订阅 `EnemyAI.OnEnemyDied` 统计击杀数，继续保持“规则主人发布事件，UI / 表现只投影”的边界。第 24 课又让 `GoldOrb.OnCollected` 进入 `GameSession.OnGoldChanged -> RunHudView`，把金币拾取事实转成本局经济 HUD；第 25 课则继续沿用 `RunResult` 快照边界，把 `BackpackValue` 放进终局结果而不是让 `ResultView` 实时反查背包。详见[目标注册表、自动武器与投射物](../../projects/backpack-survivor/target-registry-and-auto-weapon.md)、[刷怪器与对象池](../../projects/backpack-survivor/spawner-and-object-pooling.md)、[掉落系统与保底机制](../../projects/backpack-survivor/loot-drop-and-pity.md)、[拾取与磁吸](../../projects/backpack-survivor/pickup-and-magnet.md)、[背包纯数据网格](../../projects/backpack-survivor/inventory-data-grid.md)、[背包 UI 与拖拽](../../projects/backpack-survivor/inventory-ui-and-drag.md)、[掉落分层与交互拾取](../../projects/backpack-survivor/loot-layering-and-interaction.md)、[容器搜刮与宝箱系统](../../projects/backpack-survivor/container-looting-and-chests.md)、[背包交互补丁](../../projects/backpack-survivor/inventory-interaction-patches.md)、[背包武器激活](../../projects/backpack-survivor/backpack-weapon-activation.md)、[单局框架与基础 HUD](../../projects/backpack-survivor/run-session-and-basic-hud.md)、[经验成长与三选一](../../projects/backpack-survivor/level-progression-and-choice.md)、[波次导演与 15 分钟节奏](../../projects/backpack-survivor/wave-director-and-run-pacing.md)、[战斗反馈快包](../../projects/backpack-survivor/combat-feedback-pack.md)、[胜负结算与重开闭环](../../projects/backpack-survivor/run-result-and-restart-loop.md)、[构筑最小兑现](../../projects/backpack-survivor/build-payoff-dual-wield.md)和[Unity 生命周期](../../unity/lifecycle.md)。
