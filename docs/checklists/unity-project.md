...
- 尚缺证据：Profiler 前后数据、目标平台 Build、多 Collider、低帧率、缓冲区满载、切场景静态状态、跨池归还、完整调用顺序矩阵、掉落概率/保底单元测试、批量拾取副作用测试和实际 `.asmdef` / `.meta` 资产复核。
- 下一项改进：把 `DropItem.OnCollected` 接入 `InventoryGrid`，并实现背包 UI 投影与拖拽验证。
