...
- 尚缺证据：Profiler 前后数据、目标平台 Build、多 Collider、低帧率、缓冲区满载、切场景静态状态、跨池归还、完整调用顺序矩阵、掉落概率/保底单元测试、批量拾取副作用测试、拖拽中断回滚、宝箱生成、丢弃回捡、交互失败反馈、合并升级、邻接扫描、ItemView 接口点 UI 和实际 `.asmdef` / `.meta` / Prefab / Canvas / Input Actions / Layer 资产复核。
- 下一项改进：为背包武器激活、邻接效果结算、DualWield 兑现、拖拽取消和合并升级补齐可重复验证。
